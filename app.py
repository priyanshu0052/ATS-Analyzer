...
def main():
    ...
    # Sidebar
    with st.sidebar:
        st.title("📄 Resume ATS Tracker with Google Gemini Pro")
        st.header(" About ")
        st.write("""
    ...
    # Input section with validation( text area define kr rhe h )
    jd = st.text_area(
        "Job Description",
    ...
    if st.button("Analyze Resume", disabled=st.session_state.processing):
        ...
        if not uploaded_file:
            st.warning("please upload resume in pdf format.")
            return
        ...
        try:
            ...
            st.error(f"an error occured as  :{str(e)}") 
